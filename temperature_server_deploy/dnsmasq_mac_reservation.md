...
> **【2026-09-03 訂正】** 上の 2 点は誤りでした。
>
> 1. 「WiFi.config = チップごとに書換えが必要」は前提が違いました。IP を
>    **MAC から算出する (方式D)** ことで、静的宣言でありながらスケッチは
>    全チップ同一にできます。書換え負担もコピペミスも発生しません。
> 2. 「DHCP 予約なら折衝が 1 往復で完了する」も誤りです。`dhcp-host=` が
>    決めるのは「サーバーがどの IP を返すか」だけで、
>    DISCOVER/OFFER/REQUEST/ACK の往復回数は減りません。
> 3. 「電池省電力効果は誤差レベル (1-2%)」は 2026-08-21 の実機運用で否定されました。
>    **電池寿命 1 か月以上 → 2 週間強** (概ね半減)。DHCP 折衝は 1 回の起床あたり
>    3〜5 秒の追加通電を生み、2 分周期なら約 60 mAh/日を消費します。
>
> これらを踏まえ、ネットワーク層は方式D に切り替えています。
> 現行の設計根拠は `outputs/docs/デバイス識別設計.md`「なぜ DHCP 予約をやめたか」
> を参照してください。
